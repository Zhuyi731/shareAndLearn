...
function decode1(data) {
    return decode(convertBase64urlToBase64(data));
}
const mod = {
...
class Sha512 {
        ...
        while(index < length){
            ...
            if (typeof msg !== "string") {
                for(i = this.#start; index < length && i < 128; ++index){
                    blocks[i >> 2] |= msg[index] << SHIFT1[(i++) & 3];
                }
            } else {
                for(i = this.#start; index < length && i < 128; ++index){
                    let code = msg.charCodeAt(index);
                    if (code < 128) {
                        blocks[i >> 2] |= code << SHIFT1[(i++) & 3];
                    } else if (code < 2048) {
                        blocks[i >> 2] |= (192 | code >> 6) << SHIFT1[(i++) & 3];
                        blocks[i >> 2] |= (128 | code & 63) << SHIFT1[(i++) & 3];
                    } else if (code < 55296 || code >= 57344) {
                        blocks[i >> 2] |= (224 | code >> 12) << SHIFT1[(i++) & 3];
                        blocks[i >> 2] |= (128 | code >> 6 & 63) << SHIFT1[(i++) & 3];
                        blocks[i >> 2] |= (128 | code & 63) << SHIFT1[(i++) & 3];
                    } else {
                        code = 65536 + ((code & 1023) << 10 | msg.charCodeAt(++index) & 1023);
                        blocks[i >> 2] |= (240 | code >> 18) << SHIFT1[(i++) & 3];
                        blocks[i >> 2] |= (128 | code >> 12 & 63) << SHIFT1[(i++) & 3];
                        blocks[i >> 2] |= (128 | code >> 6 & 63) << SHIFT1[(i++) & 3];
                        blocks[i >> 2] |= (128 | code & 63) << SHIFT1[(i++) & 3];
                    }
                }
    ...
    hash() {
        ...
        for(let j = 32; j < 160; j += 2){
            ...
            blocks[j] = c4 << 16 | c3 & 65535;
            blocks[j + 1] = c2 << 16 | c1 & 65535;
        }
...
function stringify(obj, sep = "&", eq = "=", { encodeURIComponent =escape  } = {
}) {
    const __final = [];
    for (const entry of Object.entries(obj)){
        if (Array.isArray(entry[1])) {
            for (const val of entry[1]){
                __final.push(encodeURIComponent(entry[0]) + eq + encodeURIComponent(val));
            }
        } else if (typeof entry[1] !== "object" && entry[1] !== undefined) {
            __final.push(entry.map(encodeURIComponent).join(eq));
        } else {
            __final.push(encodeURIComponent(entry[0]) + eq);
        }
    }
...
function getEncodingPriority(encoding, accepted, index) {
    ...
    for(let i = 0; i < accepted.length; i++){
        const spec = specify1(encoding, accepted[i], index);
        if (spec && (priority.s - spec.s || priority.q - spec.q || priority.o - spec.o) < 0) {
            priority = spec;
        }
    }
...
function specify1(encoding, spec, index) {
    let s = 0;
...
function preferredEncodings(accept, provided) {
    ...
    if (!provided) {
        return accepts.filter(isQuality1).sort(compareSpecs1).map(getFullEncoding);
    }
    const priorities = provided.map(function getPriority(type, index) {
        return getEncodingPriority(type, accepts, index);
    });
    return priorities.filter(isQuality1).sort(compareSpecs1).map(function getEncoding(priority) {
        return provided[priorities.indexOf(priority)];
    });
}
...
function getLanguagePriority(language, accepted, index) {
    ...
    for(let i = 0; i < accepted.length; i++){
        const spec = specify2(language, accepted[i], index);
        if (spec && (priority.s - spec.s || priority.q - spec.q || priority.o - spec.o) < 0) {
            priority = spec;
        }
    }
...
function specify2(language, spec, index) {
...
function preferredLanguages(accept, provided) {
    ...
    if (!provided) {
        return accepts.filter(isQuality2).sort(compareSpecs2).map(getFullLanguage);
    }
    const priorities = provided.map(function getPriority(type, index) {
        return getLanguagePriority(type, accepts, index);
    });
    return priorities.filter(isQuality2).sort(compareSpecs2).map(function getLanguage(priority) {
        return provided[priorities.indexOf(priority)];
    });
}
...
function getMediaTypePriority(type, accepted, index) {
    ...
    for(let i = 0; i < accepted.length; i++){
        const spec = specify3(type, accepted[i], index);
        if (spec && (priority.s - spec.s || priority.q - spec.q || priority.o - spec.o) < 0) {
            priority = spec;
        }
    }
...
function preferredMediaTypes(accept, provided) {
    ...
    if (!provided) {
        return accepts.filter(isQuality3).sort(compareSpecs3).map(getFullType);
    }
    const priorities = provided.map(function getPriority(type, index) {
        return getMediaTypePriority(type, accepts, index);
    });
    return priorities.filter(isQuality3).sort(compareSpecs3).map(function getType(priority) {
        return provided[priorities.indexOf(priority)];
    });
}
...
function splitParameters(str) {
    ...
    for(let i1 = 0; i1 < parameters.length; i1++){
        parameters[i1] = parameters[i1].trim();
    }
    return parameters;
}
class Negotiator {
...
const path2 = isWindows ? mod1 : mod2;
...
function lookup(path) {
    const extension = extname2("x." + path).toLowerCase().substr(1);
...
class HttpError extends Error {
    ...
    constructor(code, message){
        ...
        let className = Status1[code];
        if (!className.endsWith("Error")) {
            className += "Error";
        }
        const msg = message != null ? message : STATUS_TEXT1.get(code);
        this.message = msg;
...
new RegExp("^([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\\.([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])$");
...
function fmtLong(ms) {
    ...
    if (msAbs >= 1000) {
        return plural(ms, msAbs, 1000, "second");
    }
    return ms + " ms";
}
...
function append(header, field) {
    ...
    const fields = !Array.isArray(field) ? parse5(String(field)) : field;
    ...
    let val = header;
    const vals = parse5(header.toLowerCase());
...
function is(mediaType, types) {
    ...
    for(i = 0; i < types.length; i++){
        const normalized = normalize4(type = types[i]);
...
const normalize4 = function normalize(type) {
    switch(type){
        case "urlencoded":
...
function normalizeType(value) {
    const type = parse3(value).type;
    if (!TYPE_REGEXP1.test(type.toLowerCase())) {
        return null;
    }
    return type;
}
...
class Request {
    ...
    originalUrl;
    req;
    constructor(request, proxy = false, secure = false){
        ...
        let host;
        if (this.#proxy) {
            proto = this.#serverRequest.headers.get("x-forwarded-proto")?.split(/\s*,\s*/, 1)[0] ?? "http";
            host = (this.#serverRequest.headers.get("x-forwarded-host") ?? this.#serverRequest.headers.get("host")) ?? "";
        } else {
    ...
    get hasBody() {
        return this.headers.get("transfer-encoding") !== null || !!parseInt(this.headers.get("content-length") ?? "");
    }
    get header() {
    ...
    get query() {
        ...
        for (let [k, v] of new URLSearchParams(this.URL.search)){
    ...
    get(field) {
        switch(field = field.toLowerCase()){
            case "referer":
            case "referrer":
                return this.#serverRequest.headers.get("referrer") || this.#serverRequest.headers.get("referer") || "";
            default:
    ...
    is(types) {
        ...
        return typeofrequest(this.#serverRequest.headers, arr);
    }
    get type() {
    ...
    get charset() {
        try {
            const { parameters  } = parse3(this.get("Content-Type"));
    ...
    get accept() {
        return this.#accept || (this.#accept = new Accepts(this.#serverRequest.headers));
    }
    set accept(obj) {
        this.#accept = obj;
    }
...
function isReader(value) {
    return typeof value === "object" && "read" in value && typeof value.read === "function";
}
const statusEmpty = {
    204: true,
    205: true,
...
class Response {
    ...
    set status(code) {
        ...
        assert1(Number.isInteger(code), "status code must be a number");
        assert1(code >= 100 && code <= 999, `invalid status code: ${code}`);
    ...
    toServerResponse() {
        ...
        const { headers  } = this;
        if (!(this.body || headers.has("Content-Type") || headers.has("Content-Length"))) {
...
class Keygrip {
    #keys;
    #algorithm;
    constructor(keys, algorithm = "sha256"){
        if (!keys || !(0 in keys)) {
    ...
    sign(data, key) {
        let buf;
        const sKey = (typeof key == "number" ? this.#keys[key] : key) ?? this.#keys[0];
        ...
        return mod.encode(buf);
    }
    verify(data, digest) {
        return this.index(data, digest) != -1;
    }
    index(data, digest) {
        for(var i1 = 0, l = this.#keys.length; i1 < l; i1++){
            if (this.compare(digest, this.sign(data, this.#keys[i1]))) {
                return i1;
            }
        }
        return -1;
...
class Cookies {
    ...
    set(name, value, options = {
        ...
        let { secure , maxAge , expires , sameSite , domain , path ="/" , httpOnly =true ,  } = options;
    ...
    get(name, options) {
        ...
        if (idx == -1) {
            ...
            this.set(signatureName, this.#keys.sign(data), {
                signed: false
            });
        }
        return value;
...
class Context {
    ...
    req;
    res;
    constructor(app, req, request, response){
        this.request = request;
        ...
        delegates(this, "response").method("attachment").method("redirect").method("remove").method("vary").method("has").method("set").method("append").method("flushHeaders").access("status").access("message").access("body").access("length").access("type").access("lastModified").access("etag").getter("writable");
        delegates(this, "request").method("acceptsLanguages").method("acceptsEncodings").method("acceptsCharsets").method("accepts").method("get").method("is").access("querystring").access("idempotent").access("socket").access("search").access("method").access("query").access("path").access("url").access("accept").getter("origin").getter("originalUrl").getter("href").getter("subdomains").getter("protocol").getter("host").getter("hostname").getter("URL").getter("header").getter("headers").getter("secure").getter("stale").getter("fresh").getter("ips").getter("ip");
    }
    inspect() {
        return this.toJSON();
    }
        ...
        const getStatusCode = (err)=>{
            switch(true){
                ...
                case err.status && typeof err.status === "number" && err.status <= 500:
                    return err.status;
                case err.statusCode && typeof err.statusCode === "number" && err.status <= 500:
...
class App extends EventEmitter {
    ...
    listen(options) {
        ...
        const isTls = typeof options !== "string" && typeof options.certFile !== "undefined";
        this.server = isTls ? serveTLS(options) : serve(options);
        this.secure = isTls;
    ...
    async handle(server) {
        ...
        for await (const req of server){
            ...
            const response = Object.assign(new Response(request), this.response);
            const ctx = Object.assign(new Context(this, req, request, response), {
                state: {
                }
            }, this.context);
